...
describe('HomeView 대시보드', () => {
  it('세 패널을 모두 그린다', async () => {
    ...
    render(<HomeView active backend={READY} onNavigate={vi.fn()} />)
  ...
  it('사이드카가 준비되기 전에는 요청 자체를 보내지 않는다', async () => {
    ...
    render(
      <HomeView active backend={{ state: 'starting', port: null }} onNavigate={vi.fn()} />
    )
  ...
  it('스파크라인은 30일 기록에서 마지막 7일만 쓴다', async () => {
    install()
    const { container } = render(<HomeView active backend={READY} onNavigate={vi.fn()} />)
  ...
  it('비활성 상태에서는 데이터를 부르지 않는다', async () => {
    ...
    const { fetchMock } = install()
    render(<HomeView active={false} backend={READY} onNavigate={vi.fn()} />)
  ...
  it('My DB 브리지가 아예 없어도 나머지 패널은 산다', async () => {
    ...
    const { fetchMock } = install({ todos: [todo({ id: 't1', title: '살아남아야 할 항목' })] })
  ...
  it('바로가기 버튼이 해당 화면으로 넘긴다', async () => {
    ...
    render(<HomeView active backend={READY} onNavigate={onNavigate} />)
  ...
  it('보고서는 최신 3건까지만 싣는다', async () => {
    ...
    const { container } = render(<HomeView active backend={READY} onNavigate={vi.fn()} />)
    await waitFor(() => expect(container.querySelectorAll('.home-report').length).toBe(3))
    expect(screen.getByText('보고서 0')).toBeTruthy()
    expect(screen.queryByText('보고서 3')).toBeNull()
  })
})
