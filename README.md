...
![version](https://img.shields.io/badge/version-0.1.2-F16522?style=flat-square)
...
[**⬇️ 다운로드**](#-다운로드--설치) · [**🚀 처음 설정**](#-처음-설정-first-run) · [**📖 사용법**](#-사용법) · [**🏗️ 아키텍처**](#-아키텍처)
...
- [🖼️ 스크린샷](#️-스크린샷)
- [⬇️ 다운로드 & 설치](#-다운로드--설치)
- [🚀 처음 설정 (First-run)](#-처음-설정-first-run)
- [📖 사용법](#-사용법)
- [🏗️ 아키텍처](#-아키텍처)
- [🧰 기술 스택](#-기술-스택)
- [🛠️ 소스에서 빌드](#️-소스에서-빌드)
...
| | |
|---|---|
| 🖥️ **완전 로컬** | Ollama로 LLM을 내 PC에서 구동. 코드·문서가 외부 서버로 전송되지 않고, 이용료가 없습니다. |
| 🤖 **진짜 에이전트** | 파일 읽기·쓰기·편집·삭제, 검색(glob·grep·RAG), 명령/웹 실행 등 **17개 도구**를 스스로 계획해 사용합니다. |
...
### 🎨 완성도
- 청강대학교 시그니처 **CK Orange(#F16522)** 브랜딩, 애플 감성 스프링 애니메이션
...
> 실제 실행 화면을 `docs/` 폴더에 추가한 뒤 아래 경로를 연결하세요.

| 에이전트 | 홈 (온보딩·통계) |
|:---:|:---:|
| ![agent](docs/screenshot-agent.png) | ![home](docs/screenshot-home.png) |
...
| RAM | 16 GB | 32 GB |
| GPU | 통합/CPU 실행 가능 | NVIDIA 8 GB+ VRAM |
| 필수 | [Ollama](https://ollama.com/download) 설치 | — |

### 설치 단계
1. [**Releases**](https://github.com/devbin-lab/AISO/releases/latest) 페이지에서 **`Aiso-0.1.2-Setup.exe`** 다운로드
2. 설치 파일 실행 → 설치 경로 선택 후 설치
3. Aiso 실행

> ⚠️ **SmartScreen 안내**: 현재 설치본은 코드 서명이 없어 처음 실행 시 "Windows의 PC 보호" 경고가 뜰 수 있습니다.
...
flowchart LR
    ...
    Py --> Tools["에이전트 도구 17종<br/>파일 · grep/glob · run · web"]
